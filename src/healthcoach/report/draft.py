...
def _dates_warning(findings: Sequence[Finding]) -> str | None:
    ...
    от 01.09. Это честно: даты у находок и правда разные, и «не пишите
    так, будто всё получено сегодня» здесь такая же правда, как в наборе.
    То же самое давно происходит с двумя бланками разных дат внутри
    одного среза. Находки вовсе без даты (вызовы до многосрезового
    отчёта) предупреждения по-прежнему не дают."""
