...
def generate_draft(
    ...
    """
    # Ничего к payload здесь не приписывается: сторож утечки зовётся
    # внутри `build_payload`, и текст, добавленный после него, ушёл бы
    # модели непроверенным. Предупреждение о разных датах живёт там же,
    # где собирается остальной запрос.
