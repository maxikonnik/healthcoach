...
class Section:
    ...
    instruction: str
    kinds: tuple[str, ...]
    """Виды находок, на которых стоит раздел. Пустой кортеж — раздел не
    привязан ни к одной находке; чтобы получить все виды, их нужно
    перечислить явно."""
...
_COMMON = (
    ...
    "Не давай ссылок и не приводи адресов сайтов. Не называй имён врачей и "
    "не приводи их контактов."
)
"""Общее правило для всех разделов.

Про ссылки и имена врачей сказано здесь, а не в одном разделе «врачи»:
спецификация (разделы 9 и 10) запрещает и то и другое во всём клиентском
PDF, а инструкции этого не говорили нигде. Модель получает находки, в
которых заметка коуча уже вычеркнута (`privacy/findings.py`), — но
запрет в инструкции стоит рядом с ней как второй слой: коуч правит текст
после модели, и правило должно быть видно там, где текст пишется."""
...
SECTIONS: tuple[Section, ...] = (
    Section(
        ...
        # Раздел пересказывает цели клиента его словами и не трактует ни
        # одной находки — привязывать его к находкам было бы нечем обосновать.
        kinds=(),
    ),
    Section(
        id="карта_систем",
    ...
    Section(
        ...
        kinds=(KIND_ANALYTE, KIND_DERIVED, KIND_QUESTIONNAIRE),
    ),
    Section(
        id="образ_жизни",
