...
def scale_for(
    ...
    просто прижимается к ближнему краю (`value_outside = True`); шаблон
    покажет на этом месте стрелку вместо точки.
    ...
    точка), но полоса коридора рисуется нулевой ширины. Лабораторный
