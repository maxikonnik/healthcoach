...
class AnalyteVerdict:
    ...
    сочинил код («для этого показателя целевой коридор в базе знаний не
    задан», «не удалось вычислить»), этим флагом не помечены: они безопасны и
...
def check_measurements(
    ...
    for measurement in measurements:
        ...
        if target is None:
            # Две разные причины, два разных текста: коуч читает note как
            # рабочий список, и фраза решает, что она пойдёт делать. Если
            # коридора нет вовсе ни для кого (`analyte.targets` пуст —
            # показатель заведён по корпусу без коридора, решение 1 плана),
            # сказать «не подошли пол и возраст» значило бы соврать: для
            # другого пола или возраста коридора тоже нет. Если коридор
            # есть, но ни одно условие не подошло этому измерению — вот
            # тогда причина ровно в поле и возрасте, и старый текст верен.
            note = (
                "для этого показателя целевой коридор в базе знаний не задан"
                if not analyte.targets
                else "целевой коридор задан для других условий — не для этого пола и возраста"
            )
            verdicts.append(
                AnalyteVerdict(
                    analyte_id=analyte.id,
