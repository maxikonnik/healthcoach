...
def _analyte(raw: dict) -> Analyte:
    ...
    # Показатель без целевых коридоров валиден и заводится намеренно.
    #
    # Раньше отсутствие 'целевые' было ошибкой файла. Это запрещало ровно
    # то, что нужнее всего: завести показатель, чтобы он узнавался, а
    # коридор к нему задать позже. Придумать коридор за коуча нельзя —
    # это врачебное суждение, — а без него показатель всё равно полезен:
    # название и единицы сверяются, динамика строится, а в находках он
    # получает статус «правило не задано» с поднятым rule_missing, то есть
    # попадает в список коуча «нет правила в базе знаний». Молча пустой
    # список и явно опущенный ключ значат здесь одно и то же.
    targets = tuple(_target(t, where) for t in raw.get("целевые") or ())
    analyte = Analyte(
        id=analyte_id,
