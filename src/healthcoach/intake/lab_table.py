...
ROLE_FLAG = "флаг"
"""Пометка лаборатории («выше нормы», «ниже нормы»): H, L, ↑, *, или ничего.
Опознана и не нужна — как и «прочее», — но, в отличие от него, безопасна
не только последней колонкой. Свободный текст запрещён вне последней
колонки потому, что непоследняя колонка забирает ровно один токен, а
несколько слов комментария сдвинули бы все следующие колонки; метка сама
по себе — ровно один короткий токен, и сдвигать ей нечего, поэтому она
разрешена в любом месте шапки (см. `_find_header`, `_split_row`)."""
ROLE_OTHER = "прочее"
...
_HEADER_NEAR_MISS_COLUMNS = 3
"""Сколько колонок должна назвать строка, чтобы, потеряв обязательную роль,
считаться недочитанной шапкой, а не прозой.

Считаются не слова, а разные точно опознанные роли: «Ед. изм.» — два слова
одной колонки, а не две колонки, и неточное совпадение здесь не в счёт
(именно оно и цепляется за прозу: «исследования» в фразе «результат
исследования» отличается от известного слова на одну букву).

Три, а не две. Две роли складываются в обычной русской фразе бланка —
«динамика значений», «результат исследования»: назвать такую строку шапкой
значило бы объявить прозу таблицей и отказать коучу словами про колонки
там, где никаких колонок нет. Три разные роли подряд в прозе не
встречаются — так пишут только шапку. Одна роль тем более ничего не значит:
слово «показатель» стоит в половине заключений.

Порог сознательно строгий с обеих сторон: ошибка в одну сторону — коуч
получает общий отказ «не похоже на таблицу» вместо точного (это сегодняшнее
поведение, оно не хуже прежнего), в другую — заключение УЗИ объявляется
сломанной таблицей анализов и коуч ищет несуществующую колонку."""
...
_FLAG_TOKEN = re.compile(r"^(?:[HLNAСВНKК]|↑|↓|↔|\*|\+\+?|--?)$")
"""Как выглядит пометка лаборатории («выше/ниже нормы»), стоящая одним
токеном.

Список закрытый и короткий: латинские H/L/N/A международного образца,
одиночные русские С/В/Н/К, значки направления и знаки отклонения. Ровно
то, чем бланк помечает выход за норму, — и ничего больше.

Соблазн описать пометку формой («одна-две заглавные буквы») стоил бы
дорого: под такое правило попадают аббревиатуры в названиях — ЛГ, АТ,
ХГ, ТВ, — и последнее слово названия молча становилось бы пометкой, а
показатель сохранялся под обрубленным именем. В корпусе образцов колонка
«Флаг» у всех строк пуста, и единственным, что подошло под правило по
форме, оказалось «ТВ» — тромбиновое время, а не пометка.

Колонка «флаг» безопасна в середине шапки только пока не может забрать
больше одного короткого токена (см. `_split_row`)."""
...
_SERVICE_ID = re.compile(
    r"(?:\bкарта\b|\bИБ\s*№|\bИ/б\s*№|\bистория\s+болезни\b)[\s:.№-]*\d",
    re.IGNORECASE,
)
"""Номер карты или истории болезни — где угодно в строке, не только с её
начала.

Два других класса отсева требуют метку в начале строки, и здесь это не
работает: бланк печатает сначала фамилию пациента и только потом номер
(«ФАМИЛИЯ ИМЯ ИБ №: 1234 …»). Такая строка разбирается *успешно* — номер
истории болезни становится значением, — и в базу попадает измерение,
названное фамилией клиента, а в отчёт табло эта фамилия печатается как
нераспознанный показатель, хотя README обещает обратное.

Совпадение только по слову целиком и только когда следом идёт число:
без границы слова «иб» находится внутри обычных слов, а без числа «карта»
могла бы оказаться частью названия. Показателя с именем «карта» или «ИБ»
не бывает — потерять результат это правило не может."""
...
def _find_header(lines: Sequence[str]) -> tuple[int, list[_HeaderColumn]]:
    ...
    «Флаг» — колонка того же рода, что «прочее» (опознана, но не нужна),
    но не того же поведения: место в её ячейке — не проза, а одна короткая
    пометка лаборатории или пустота, и она разрешена в любой позиции, не
    только последней. Опасность свободного текста здесь просто не
    применима: непоследняя колонка и так забирает ровно один токен, а
    метка ровно один токен и занимает. `_split_row` дополнительно
    проверяет вид самого токена — это не смягчение правила, а его
    соблюдение для колонки, чьё содержимое устроено иначе.

    Обязательная роль, которой не хватило, — не всегда «это не таблица».
    Строка, назвавшая три колонки точно (`_HEADER_NEAR_MISS_COLUMNS`) и
    потерявшая обязательную, — это шапка, которую разбор не дочитал:
    например, колонку названия лаборатория озаглавила словом, которого нет
    в словаре. Такую строку разбирать всё равно нельзя (колонка встанет не
    на своё место), но и молчать о ней нельзя: отказ называет непонятое
    слово, как и отказ по нераспознанным словам выше. Отказ откладывается до
    конца поиска: настоящая шапка может стоять ниже недочитанной строки, и
    отказать сразу значило бы потерять документ, который разбирается.
    ...
    (`_MESSAGE_EMPTY_DOCUMENT`).
    """
    near_miss: tuple[str, str, list[str]] | None = None
    for index, line in enumerate(lines):
        if _HAS_DIGIT.search(line):
            continue
        columns = _header_columns(line)
        exact_roles = [column.role for column in columns if column.exact]
        missing = [role for role in _ROLES_MANDATORY if role not in exact_roles]
        if missing:
            if near_miss is None and len(exact_roles) >= _HEADER_NEAR_MISS_COLUMNS:
                near_miss = (
                    line.strip(),
                    missing[0],
                    _unrecognised_header_words(line),
                )
            continue
    ...
    if near_miss is not None:
        header, missing_role, unrecognised = near_miss
        unknown = (
            f"нераспознанные слова: {', '.join(unrecognised)}"
            if unrecognised
            else "все её слова опознаны, а колонки не нашлось"
        )
        raise LabTableError(
            f"строка-шапка {header!r} прочитана не до конца: колонка "
            f"{missing_role!r} в ней не найдена ({unknown}) — разбирать "
            "дальше нельзя, колонка встанет не на своё место"
        )
...
def _is_service_line(line: str) -> bool:
    ...
    Три класса, и ни один не судит по одному лишь первому слову: строка,
    ...
    Первые два ищут метку в начале строки, третий — номер карты или
    истории болезни где угодно в ней: перед ним бланк печатает фамилию,
    и от начала строки такую метку не поймать.
    """
    return bool(
        _SERVICE_PHRASE.match(line)
        or _SERVICE_LABEL.match(line)
        or _SERVICE_ID.search(line)
    )
...
def _split_row(line: str, roles: Sequence[str]) -> LabRow | None:
    ...
    name_end = first
    if ROLE_FLAG in tail_roles and tail_roles.index(ROLE_FLAG) < tail_roles.index(
        ROLE_VALUE
    ):
        # На бланке флаг печатается между названием и числом — раньше
        # значения, а не после него, как любая другая колонка. Здесь, и
        # только здесь, `rest` (он начинается с числа-значения) не
        # выравнен с порядком колонок шапки: колонку перед значением
        # отделяет не токен-в-очереди, а сам разбор имени. Токен перед
        # числом достаётся флагу, только если похож на пометку лаборатории
        # — короткую и без строчных букв; длинное слово там — просто
        # последнее слово многословного названия, а не флаг с опечаткой,
        # и его трогать нельзя.
        if first >= 2 and _FLAG_TOKEN.match(tokens[first - 1]):
            name_end = first - 1
        tail_roles = [role for role in tail_roles if role != ROLE_FLAG]

    name = " ".join(tokens[:name_end])
    flag_value = tokens[name_end] if name_end < first else ""
    rest = tokens[first:]
    fields: dict[str, str] = {ROLE_NAME: name, ROLE_FLAG: flag_value}
    ...
    for index, role in enumerate(tail_roles):
        if not rest:
            ...
            if role not in (ROLE_OTHER, ROLE_FLAG):
                return None
            fields[role] = ""
            continue
        if index == len(tail_roles) - 1:
            if role in (ROLE_UNITS, ROLE_FLAG) and len(rest) > 1:
                return None
