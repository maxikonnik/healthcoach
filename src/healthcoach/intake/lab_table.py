...
_SERVICE = re.compile(
    r"^\s*("
    r"Дата исследования|Штрихкод|Материал|Вн\.№"
    r"|Страница\b|№\s*заказа\b|Адрес\b|ПЕЧАТЬ:|Дата рождения\b"
    r"|I{1,3}\s*триместр:"
    r")"
)
"""Строки бланка, которые не являются показателем ни при каком раскладе —
служебная разметка формы, а не результат анализа. Список собран по отчёту
табло корпуса образцов (Task 3 плана), а не придуман заранее: «Страница»
попадала в измерения как строка с именем «Страница» (число страницы читалось
как значение), «Адрес пациента …» — с настоящим домашним адресом в имени
показателя. `\\b` на конце слова — не украшение: префиксное совпадение
проглотило бы и настоящую строку бланка, которая начинается на те же буквы,
но служебной не является (см. `test_service_word_match_is_whole_word_not_a_string_prefix`
в test_lab_table.py — оттого в списке нет «Пациент», «ФИО», «Врач»,
«Организация»: по отчёту табло ни одна не встречена отдельной строкой, а
«Пациент» как префикс съел бы «Пациентка беременна»)."""
