...
_FLAG_TOKEN = re.compile(r"^[A-ZА-ЯЁ↑↓↔*]{1,2}$")
"""Как выглядит пометка лаборатории («выше/ниже нормы»), стоящая одним
токеном: заглавная буква-другая (H, L, критично латиница — колонка
международного образца) или значок направления. Строчных букв здесь не
бывает: словом, а не пометкой, была бы уже любая обычная часть названия
показателя. Проза сюда не проходит — то и требуется: колонка «флаг»
безопасна в середине шапки только пока не может забрать больше одного
короткого токена (см. `_split_row`)."""
