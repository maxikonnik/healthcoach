...
**Укрепление обезличивания — отложено решением партнёра.** Сторож работает и ловит прямые совпадения, но ревью нашло пять способов протащить имя мимо него, и все они остаются открытыми:

1. Контакты стерегутся дословно. `+7 (916) 123-45-67` проходит, если сохранено `+7 916 123-45-67`; извлечение цифровых последовательностей вообще не срабатывает на форматированном номере.
2. Телеграм-ник без собаки проходит.
3. Латинская транслитерация имени проходит — а лаборатории печатают фамилию именно так, и текст бланка коуч вставляет.
4. Клиент, у которого все части имени короче четырёх букв, не защищён вовсе: для «Цой Лев Ким» список основ пуст, и сторож держит только код клиента и дату рождения.
5. Код клиента без дефиса, перевод строки внутри основы и подмена кириллической буквы латинской внутри основы проходят.

Направление правки известно: нормализовать обе стороны перед сравнением (регистр, схлопывание похожих букв кириллицы и латиницы, отбрасывание всего, кроме букв и цифр), добавить транслитерацию основ из существующей таблицы `_TRANSLIT` в `knowledge/import_xlsx.py`, добавить цифровую форму телефона, и короткие части имени не пропускать, а искать по границе слова.

Сознательно не чиним: инициалы «К. Е. В.» — однобуквенная строка отвергала бы любой текст; беглые гласные («Козел» → «Козла») — ловить их значит гадать о склонении.

**Цена ложных отказов, измеренная на живой базе знаний.** С названиями показателей и заголовками блоков опросника коллизий нет. По тексту вопросов: клиент по фамилии Кровин отвергнет любой текст про кровь и отчёт ему собрать нельзя; Костин спотыкается на «ломкость» и «жидкость»; Сахаров — на «сахар крови». Выключателя у сторожа нет по замыслу; сообщение называет виновную основу, так что коуч понимает происходящее. Решение о том, что с этим делать, за партнёром.

**Что смягчает отсрочку.** Свободный текст клиента коуч вычитывает сам перед отправкой — сторож стоит за вычиткой, а не вместо неё. Остальное, что уходит модели, идёт из посчитанных кодом находок и справочника специальностей, где имён клиента нет. Риск станет настоящим, если вычитку уберут или автоматизируют.

**Фаза цикла читается, но нигде не записывается.** `Subject.cycle_phase` доходит до двух мест: `select_target` сверяет по ней условие целевого коридора, `llm/payload.py` выводит её строкой «фаза цикла». Записать её в `src/` нечем: ни колонки в базе, ни поля в форме, ни поля в хранилище клиента — единственное место, где она вообще выставлена, это тест, который конструирует `Subject` напрямую. Значит условие `фаза_цикла` в файле референсов — мёртвое: оно никогда не совпадёт, целевой коридор с ним будет молча пропущен, и коуч об этом не узнает. Оба места чтения оставлены как есть — они верны и ничего не стоят. Чтобы фаза заработала, нужно поле в карточке клиента; как оно должно выглядеть на экране — решение партнёра, здесь оно не придумывается. До тех пор условие `фаза_цикла` в базу знаний заводить не следует.
