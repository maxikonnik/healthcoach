...
**Модель не знает о графике, который печатается рядом с её текстом.** Найдено сквозным прогоном после слияния. График динамики строится по всей истории клиента — две точки за два среза, — а находки для модели собираются по одному срезу, где измерение одно. Модель честно пишет «это первый срез, сравнивать не с чем», и рядом печатается график с двумя точками. Ни одно число не искажено: и текст, и график по отдельности верны, расходится их толкование. Чинится передачей рядов динамики в запрос к модели, чтобы раздел «Что изменилось» опирался на то же, что нарисовано. Требует решения, как показывать ряд модели и что делать, когда точек много.
