...
**Чего разбор не различает.** Строка без числа считается началом перенесённого названия и ждёт следующую строку. Абзац врачебных пояснений выглядит для разбора так же, поэтому не попадает ни в записи, ни в `unparsed`. Отличить прозу от перенесённого названия по одному тексту нельзя, а решение партнёра — не терять данные, а не угадывать их. Строка вроде `Биохимия 21 показатель (расширенная)` при этом может стать записью с бессмысленным значением: она дойдёт до коуча в свёрнутом списке нераспознанных и будет там видна. Это шум, а не молчаливая ошибка, и потому допустим.
...
def parse_lab_lines(lines: Sequence[str]) -> LabTable:
    ...
    for line in lines:
        ...
        if row is not None:
            rows.append(row)
            pending_name = ""
        elif not _STARTS_WITH_NUMBER.match(cleaned) and cleaned:
