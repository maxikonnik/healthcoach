...
def test_corpus_scoreboard_meets_baseline(samples_dir):
    ...
    #
    # Фикс-раунд, пункт 1: «Флаг» заведён отдельной ролью и разрешён не
    # только последней колонкой (на бланке он стоит между названием и
    # результатом) — шести документам он был единственной причиной отказа
    # «шапка-колонки». Три перешли в accepted напрямую (40 → 43), два —
    # в confirmable (у них ещё и опечатка «Ел.» вместо «Ед.», 2 → 4), один
    # остался отказом по не связанной с флагом причине (OCR-опечатка в
    # слове «исследования», далёкая от словаря). Строки этих документов
    # раньше не разбирались вовсе (шапка отказывала раньше первой строки);
    # сопоставлено 537 → 567, единицы сошлись у 460 → 478.
    min_accepted = 43
    min_confirmable = 4
    min_resolved = 567
    min_units_agreed = 478
